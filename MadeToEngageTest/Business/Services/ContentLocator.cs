...
{
    [ServiceConfiguration(ServiceType = typeof(IContentLocator))]
    public class ContentLocator : IContentLocator
    {
        ...
        {
            var eventChildPages = _contentLoader.GetChildren<EventPage>(parent);
            return eventChildPages.OrderBy(x => x.StartDate);
        }
    }
}
